 ...
 **********************************************************************************************************************/
extern void r8_nest_probe(uint32_t depth) __attribute__((weak));
extern void r8_bad_intid(uint32_t intid) __attribute__((weak));
...
void bsp_common_interrupt_handler (uint32_t id)
{
    ...
    if (BSP_GIC_SPECIAL_ID != gic_intid)
    {
        ...
        /* ---- 巢狀深度探針(2026-08-25)---------------------------------
         * 下面那行 __enable_irq() 讓中斷可以巢狀,而上面這個陣列只有
         * BSP_PRV_INTERRUPTABLE_NUM(32)格、指標是 uint8_t,**沒有邊界檢查**。
         * 深度真的會不會超過 32?先量再說,這一版不改行為。
         * weak 宣告:沒有定義探針的韌體照樣連結得起來。 */
        if (NULL != r8_nest_probe)
        {
            r8_nest_probe((uint32_t) g_current_interrupt_pointer);
        }
        ...
        /* ⚠️ 2026-08-27 修:索引必須先檢查邊界。
         * 原本直接 g_vector_table[gic_intid]() —— gic_intid 只被遮罩到
         * 0..1023(BSP_PRV_ID_MASK),但這張表只有 BSP_ICU_VECTOR_MAX_ENTRIES 格。
         * 實測在 RZ/V2H CR8 上抓到 ICCIAR = 876:
         *     g_vector_table[876] 落在 g_gpt5_extend 裡,讀到 0x00010001,
         *     被當成函式指標呼叫 -> 跳進 ITCM 0x10000 執行任意內容。
         * 後果是 sp 變成任意值,退出路徑的 LDM 當場 Data Abort。
         * 越界就不派送,只記錄;portASM 後續仍會寫 ICCEOIR,GIC 那側正常收尾。 */
        if (gic_intid < BSP_ICU_VECTOR_MAX_ENTRIES)
        {
            /* Branch to an interrupt handler. */
            g_vector_table[gic_intid]();
        }
        else if (NULL != r8_bad_intid)
        {
            r8_bad_intid((uint32_t) gic_intid);
        }
        else
        {
            /* 沒有探針時就單純跳過 */
