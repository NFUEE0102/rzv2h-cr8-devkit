...
- **Address `0x439F0000`** (core0; core1 uses `0x441F0000`, the CM33 and the
  legacy shared layout use `0x431F0000` — `bb.py` auto-detects all three) —
  tail of the `vring-ctl1` window: already UIO-mapped
...
5. Stack watermarks sane (the reference firmware stays below 5% of SVC).
6. For long soaks: `boot_count` unchanged (no watchdog rescues) over the window.

## Design notes: which fields to trust
...
- On imprecise (async) external aborts, DFAR and LR are decoys. Use the
  per-interrupt ring buffer and the independent tick counter to localize the
  fault instead.
